...
const UserRow = ({ user, navigation, deleteUser }) => {
    ...
    return (
        <TouchableOpacity onPress={() => setShowDesc(!showDesc)}>
            <View style={{ flexWrap: 'wrap', padding: 10, maxHeight: 500, display: "flex", margin: 10, flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'center', backgroundColor: 'white' }}>
                ...
                {showDesc ?
                    <View style={{ width: "100%", display: "flex", alignItems: 'flex-end', justifyContent: 'space-between' }}>
                        <Text style={{ alignSelf: 'flex-start' }}>{user.biographie}</Text>
                        <View style={{ flexDirection: 'row' }}>
                            <TouchableOpacity onPress={() => navigation.navigate('ReponsesFromUser', { id: user.id })} style={{ width: 20 }}><Ionicons name="help-circle" size={18} color="blue" /></TouchableOpacity>
